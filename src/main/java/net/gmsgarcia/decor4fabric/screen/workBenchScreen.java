package net.gmsgarcia.decor4fabric.screen;
...
public class workBenchScreen extends HandledScreen<workBenchScreenHandler> {
    ...
    protected void drawMouseoverTooltip(MatrixStack matrices, int x, int y) {
        ...
        if (this.canCraft) {
            ...
            List<workBenchRecipe> list = ((workBenchScreenHandler)this.handler).getAvailableRecipes();
            for (int l = this.scrollOffset; l < k && l < ((workBenchScreenHandler)this.handler).getAvailableRecipeCount(); ++l) {
    ...
    private void renderRecipeBackground(MatrixStack matrices, int mouseX, int mouseY, int x, int y, int scrollOffset) {
        for (int i = this.scrollOffset; i < scrollOffset && i < ((workBenchScreenHandler)this.handler).getAvailableRecipeCount(); ++i) {
            int j = i - this.scrollOffset;
            int k = x + j % 4 * 16;
            int l = j / 4;
            int m = y + l * 18 + 2;
            int n = this.backgroundHeight;
            if (i == ((workBenchScreenHandler)this.handler).getSelectedRecipe()) {
                n += 18;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (this.canCraft) {
            ...
            for (int l = this.scrollOffset; l < k; ++l) {
                ...
                if (!(d >= 0.0) || !(e >= 0.0) || !(d < 16.0) || !(e < 18.0) || !((workBenchScreenHandler)this.handler).onButtonClick(this.client.player, l)) continue;
                MinecraftClient.getInstance().getSoundManager().play(PositionedSoundInstance.master(SoundEvents.UI_STONECUTTER_SELECT_RECIPE, 1.0f));
                this.client.interactionManager.clickButton(((workBenchScreenHandler)this.handler).syncId, l);
                return true;
            }
            i = this.x + 119;
            j = this.y + 9;
    ...
    private boolean shouldScroll() {
        return this.canCraft && ((workBenchScreenHandler)this.handler).getAvailableRecipeCount() > 12;
    }

    protected int getMaxScroll() {
        return (((workBenchScreenHandler)this.handler).getAvailableRecipeCount() + 4 - 1) / 4 - 3;
    }

    private void onInventoryChange() {
        this.canCraft = ((workBenchScreenHandler)this.handler).canCraft();
        if (!this.canCraft) {
